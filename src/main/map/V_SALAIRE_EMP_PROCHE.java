...
public class V_SALAIRE_EMP_PROCHE extends EMP {
    ...
    //fonction findEmpObsolète : 
    public static Vector<Object> findEmp(String date,Object where ) throws SQLException, ReflectiveOperationException{
        Connection conn=DBconnect.connect();
        Vector<Object> results=DBClassManager.findObject0(SpecialQueryBuilder.getPstmtFor_SALAIRE_EMP(conn, date,"V_SALAIRE_EMP_PROCHE"), where );
        conn.commit();
        conn.close();
        return  results;
    }
    public  Vector<Object> findEmp(String date) throws SQLException, ReflectiveOperationException{
        Connection conn=DBconnect.connect();
        Vector<Object> results=DBClassManager.findObject0(SpecialQueryBuilder.getPstmtFor_SALAIRE_EMP(conn, date,"V_SALAIRE_EMP_PROCHE"), this );
        conn.commit();
        conn.close();
        return  results;
    }
